#!/usr/bin/env bash
# Stage 5 — SCENIC+ GRN inference → eRegulons + per-cell activity.
#   5a TF_to_gene      -> tf_to_gene_adj.tsv    (GBM co-expression, 756 cistrome TFs)
#   5b region_to_gene  -> region_to_gene_adj.tsv (GBM importance + SR correlation)
#   5c eGRN            -> eRegulon.tsv           (direct cistromes; real full-pipeline params)
#   5d AUCell          -> AUCell.h5mu            (per-cell eRegulon activity, for Stage 6)
# Verified CLI pattern from the epics Tier-2 test (scripts/test03_run_grn.sh); here real params.
# Run: bash scripts/17_run_grn.sh
set -euo pipefail
...
TF_NAMES=${WORK}/tf_names_menr.txt                        # Stage 4d (756 cistrome TFs)
