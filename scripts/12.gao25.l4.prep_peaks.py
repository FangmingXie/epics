#!/usr/bin/env python
"""Stage 3a (gao25 · l4) — L4/5-accessible peak matrix from the precomputed ATAC count matrix.

Fork of scripts/12.gao25.prep_peaks_matrix.py for the L4 layer (plan/06). Method and the 5% peak
filter are unchanged; only the barcode/BED/npz paths point at the l4 namespace. The DevVIS ATAC h5ad
ships a peaks×cells count matrix (882,075 fixed-width peaks), so there is no peak calling. We:
  1. read the ATAC h5ad (backed) and subset rows to the final multiome L4/5 barcodes (ATAC _index),
  ...
  4. write the L4/5 peak BED (for the cisTarget DB build) and a regions×cells CSR npz (for cisTopic).

Run in the epics env:
  conda run --no-capture-output -n epics python -u scripts/12.gao25.l4.prep_peaks.py
"""
import numpy as np
...
L4_BARCODES = f"{DATA}/vis_l4_barcodes.txt"                   # Stage 1 (common key == ATAC _index)
WORK = f"{DATA}/work"
L4_PEAKS_BED = f"{WORK}/vis_l4_peaks.bed"                     # output: chr\tstart\tend (for DB build)
L4_MATRIX_NPZ = f"{WORK}/vis_l4_matrix.npz"                  # output: regions×cells CSR + names (cisTopic)

MIN_CELL_FRAC = 0.05                                          # keep peaks accessible in >= 5% of L4/5 cells


def main():
    valid_bc = set(open(L4_BARCODES).read().split())
    print(f"[Stage 3a gao25·l4] {len(valid_bc):,} target L4/5 barcodes")

    print(f"[Stage 3a gao25·l4] opening {ATAC_H5AD} (backed)")
    ...
    n_cells = X.shape[0]
    print(f"  L4/5 submatrix: {X.shape[0]:,} cells x {X.shape[1]:,} peaks; nnz={X.nnz:,}")
    ...
    with open(L4_PEAKS_BED, "w") as fh:
        fh.writelines(bed_lines)
    print(f"  wrote {L4_PEAKS_BED} ({n_keep:,} peaks)")
    ...
    print(f"  wrote {L4_MATRIX_NPZ}")
    print("[Stage 3a gao25·l4] complete.")
