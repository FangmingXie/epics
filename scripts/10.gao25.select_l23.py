#!/usr/bin/env python
"""Stage 1 (gao25) — Select cortical L2/3 IT CTX Glut + multiome barcodes (Allen DevVIS, mouse).

Adapts scripts/10.v2.select_it.py to the mouse DevVIS multiome (plan/05). Key simplifications:
gao25's RNA `X` is already int32 raw counts and `var_names` are already mouse gene symbols, so the
counts-slot scan and Ensembl->symbol remap collapse to validation + duplicate-symbol dedup.

Scope (confirmed): `subclass_label == "L2/3 IT CTX Glut"` AND `roi in VIS_ROIS` (5 cortical/visual
ROIs; drop the non-cortical TH/CH-BS tail). Final multiome cells = RNA cellNames ∩ ATAC _index
(the common pairing key). We standardize the RNA obs_names to `cellNames` so cisTopic (named by
ATAC `_index`) and this RNA h5ad reconcile after the ___gao25l23 suffix in Stage 4.

Run in the epics env:
  conda run --no-capture-output -n epics python -u scripts/10.gao25.select_l23.py
"""
import h5py
import numpy as np
import pandas as pd
...
L23_GEX_H5AD = f"{DATA}/vis_l23_gex.h5ad"                      # output: cortical L2/3, raw counts in .X
L23_BARCODES = f"{DATA}/vis_l23_barcodes.txt"                 # output: final RNA∩ATAC barcodes (common key)

# --- selection criteria ---
SUBCLASS_COL = "subclass_label"
L23_SUBCLASS = "L2/3 IT CTX Glut"
ROI_COL = "roi"
...
MIN_INTERSECT = 15000           # fail-fast floor (plan: expect ≈ 21k)
...
def read_atac_l23_barcodes():
    """Read ATAC obs only (h5py) and return the set of _index barcodes for cortical L2/3 cells."""
    def col(f, name):
        g = f["obs"][name]
    ...
    with h5py.File(ATAC_H5AD, "r") as f:
        ...
        roi = col(f, ROI_COL)
    mask = (sub == L23_SUBCLASS) & np.isin(roi, VIS_ROIS)
    print(f"  ATAC cortical L2/3 cells: {int(mask.sum()):,} (of {len(idx):,})")
    return set(idx[mask])


def main():
    print(f"[Stage 1 gao25] reading ATAC obs from {ATAC_H5AD}")
    atac_bc = read_atac_l23_barcodes()

    print(f"[Stage 1 gao25] reading {RNA_H5AD}")
    ...
    mask = (adata.obs[SUBCLASS_COL].astype(str) == L23_SUBCLASS) & \
           (adata.obs[ROI_COL].astype(str).isin(VIS_ROIS))
    it = adata[mask.values].copy()
    print(f"  RNA cortical L2/3 cells: {it.n_obs:,}")
    ...
    if not it.obs_names.is_unique:
        raise ValueError("RNA cellNames are not unique after L2/3 subset — cannot use as multiome key.")

    # intersect RNA cellNames ∩ ATAC _index -> final multiome cells
    rna_bc = set(it.obs_names)
    inter = rna_bc & atac_bc
    print(f"  RNA-only L2/3: {len(rna_bc):,}  ATAC-only L2/3: {len(atac_bc):,}  intersection: {len(inter):,}")
    ...
    it.write_h5ad(L23_GEX_H5AD)
    print(f"  wrote {L23_GEX_H5AD}")

    pd.Series(it.obs_names).to_csv(L23_BARCODES, index=False, header=False)
    print(f"  wrote {L23_BARCODES} ({it.n_obs:,} barcodes)")
