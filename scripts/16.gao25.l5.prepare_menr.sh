#!/usr/bin/env bash
# Stage 4 (gao25 · l5) — SCENIC+ prepare_data + motif enrichment → MuData, search space, cistromes.
# Fork of scripts/16.gao25.prepare_menr.sh for the L5 layer (plan/06). Method unchanged; only the gex,
# cisTarget DB prefix, and the ___gao25l5 barcode suffix differ. The mm10 GRCm38 gene annotation and
...
# Run: bash scripts/16.gao25.l5.prepare_menr.sh   (all steps use the epics env internally)
set -euo pipefail
...
L5_GEX_H5AD=${DATA}/vis_l5_gex.h5ad                       # Stage 1 (raw counts in .X, .raw cleared)
...
PROJECT_SUFFIX=___gao25l5                                 # cisTopic appends this to ATAC barcodes
N_CPU=8
SP="conda run --no-capture-output -n epics scenicplus"

# --- fail-fast ---
for f in "${CISTOPIC_OBJ}" "${L5_GEX_H5AD}" "${GENE_ANNOT}" "${MM10_CHROMSIZES_SP}" "${CTX_RANKINGS}" "${MOTIF_TBL_MGI}"; do
...
# =============================================================================
# 4a — MuData (multiome, per-cell). RNA barcodes get the ___gao25l5 suffix to match ATAC.
...
echo "[$(date)] Stage 4 gao25·l5 complete:"
