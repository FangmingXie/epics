#!/usr/bin/env python
"""Stage 7 (gao25) — Organize SCENIC+ eRegulons into a clean regulon -> gene table (mouse L2/3).

Reuses scripts/19.v2.organize_regulons_it.py; logic unchanged, repointed at the gao25 work dir.
From the Stage 5 direct/extended eRegulon tables, build one long table where each row is a
(regulon, gene) pair, after:
...
Run: conda run --no-capture-output -n epics python -u scripts/19.gao25.organize_regulons.py
"""
import argparse
import os
import sys
