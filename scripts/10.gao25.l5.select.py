#!/usr/bin/env python
"""Stage 1 (gao25 · l5) — Select cortical L5 IT CTX Glut + multiome barcodes (Allen DevVIS, mouse).

Fork of scripts/10.gao25.l4.select.py for the deeper IT layer L5 (plan/06). Method is unchanged; only
...
Scope: `subclass_label == "L5 IT CTX Glut"` AND `roi in VIS_ROIS` (5 cortical/visual ROIs). Final
multiome cells = RNA cellNames ∩ ATAC _index (the common pairing key). We standardize the RNA obs_names
to `cellNames` so cisTopic (named by ATAC `_index`) and this RNA h5ad reconcile after the ___gao25l5
suffix in Stage 4.

Run in the epics env:
  conda run --no-capture-output -n epics python -u scripts/10.gao25.l5.select.py
"""
import h5py
import numpy as np
import pandas as pd
...
L5_GEX_H5AD = f"{DATA}/vis_l5_gex.h5ad"                        # output: cortical L5, raw counts in .X
L5_BARCODES = f"{DATA}/vis_l5_barcodes.txt"                    # output: final RNA∩ATAC barcodes (common key)

# --- selection criteria ---
SUBCLASS_COL = "subclass_label"
L5_SUBCLASS = "L5 IT CTX Glut"
ROI_COL = "roi"
...
MIN_INTERSECT = 5000            # fail-fast floor (plan: expect ≈ 8k)
...
def read_atac_l5_barcodes():
    """Read ATAC obs only (h5py) and return the set of _index barcodes for cortical L5 cells."""
    def col(f, name):
        g = f["obs"][name]
    ...
    with h5py.File(ATAC_H5AD, "r") as f:
        ...
        roi = col(f, ROI_COL)
    mask = (sub == L5_SUBCLASS) & np.isin(roi, VIS_ROIS)
    print(f"  ATAC cortical L5 cells: {int(mask.sum()):,} (of {len(idx):,})")
    return set(idx[mask])


def main():
    print(f"[Stage 1 gao25·l5] reading ATAC obs from {ATAC_H5AD}")
    atac_bc = read_atac_l5_barcodes()

    print(f"[Stage 1 gao25·l5] reading {RNA_H5AD}")
    ...
    mask = (adata.obs[SUBCLASS_COL].astype(str) == L5_SUBCLASS) & \
           (adata.obs[ROI_COL].astype(str).isin(VIS_ROIS))
    it = adata[mask.values].copy()
    print(f"  RNA cortical L5 cells: {it.n_obs:,}")
    ...
    if not it.obs_names.is_unique:
        raise ValueError("RNA cellNames are not unique after L5 subset — cannot use as multiome key.")

    # intersect RNA cellNames ∩ ATAC _index -> final multiome cells
    rna_bc = set(it.obs_names)
    inter = rna_bc & atac_bc
    print(f"  RNA-only L5: {len(rna_bc):,}  ATAC-only L5: {len(atac_bc):,}  intersection: {len(inter):,}")
    ...
    it.write_h5ad(L5_GEX_H5AD)
    print(f"  wrote {L5_GEX_H5AD}")

    pd.Series(it.obs_names).to_csv(L5_BARCODES, index=False, header=False)
    print(f"  wrote {L5_BARCODES} ({it.n_obs:,} barcodes)")
