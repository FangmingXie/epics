#!/usr/bin/env bash
# Stage 3b (gao25) — Build a custom cisTarget motif database on the L2/3-accessible mouse peaks.
# Adapts scripts/13.v2.build_ctx_db_it.sh; the build is species-neutral, so ONLY the genome
# fasta/chromsizes change to mm10 (the motif singletons dir is shared; the MGI table is used later in
# Stage 4, not here). Built ON the L2/3 peak BED (Stage 3a) with a NEW prefix gao25_l23.
# MUST run inside the `epics` env (bedtools + cbust + pyarrow):
#   conda run --no-capture-output -n epics bash scripts/13.gao25.build_ctx_db.sh
set -euo pipefail
...
L23_PEAKS_BED=${WORK}/vis_l23_peaks.bed                # Stage 3a output (chr\tstart\tend)
...
CTX_DB_PREFIX=${DB}/gao25_l23                          # → *.regions_vs_motifs.{rankings,scores}.feather
...
for f in "${MM10_FASTA}" "${MM10_CHROMSIZES}" "${L23_PEAKS_BED}"; do
...
# --- padded-background FASTA for the L2/3 regions ---
echo "[$(date)] building padded-bg FASTA (${BG_PADDING} bp) for $(wc -l < "${L23_PEAKS_BED}") regions -> ${OUT_FASTA}"
"${DB_BUILDER}/create_fasta_with_padded_bg_from_bed.sh" \
    "${MM10_FASTA}" "${MM10_CHROMSIZES}" "${L23_PEAKS_BED}" \
...
echo "[$(date)] Stage 3b gao25 complete:"
