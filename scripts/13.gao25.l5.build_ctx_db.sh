#!/usr/bin/env bash
# Stage 3b (gao25 · l5) — Build a custom cisTarget motif database on the L5-accessible mouse peaks.
# Fork of scripts/13.gao25.build_ctx_db.sh for the L5 layer (plan/06). The build is species-neutral;
# only the peak BED and the output DB prefix change. Built ON the L5 peak BED (Stage 3a) with a NEW
# prefix gao25_l5 in the per-layer db dir (do NOT share the L2/3 DB — each layer builds on its own peaks).
# MUST run inside the `epics` env (bedtools + cbust + pyarrow):
#   conda run --no-capture-output -n epics bash scripts/13.gao25.l5.build_ctx_db.sh
set -euo pipefail
...
L5_PEAKS_BED=${WORK}/vis_l5_peaks.bed                  # Stage 3a output (chr\tstart\tend)
...
OUT_FASTA=${DB}/l5_regions_1kb_bg.fa                   # padded-background region FASTA
CTX_DB_PREFIX=${DB}/gao25_l5                           # → *.regions_vs_motifs.{rankings,scores}.feather
...
for f in "${MM10_FASTA}" "${MM10_CHROMSIZES}" "${L5_PEAKS_BED}"; do
...
# --- padded-background FASTA for the L5 regions ---
echo "[$(date)] building padded-bg FASTA (${BG_PADDING} bp) for $(wc -l < "${L5_PEAKS_BED}") regions -> ${OUT_FASTA}"
"${DB_BUILDER}/create_fasta_with_padded_bg_from_bed.sh" \
    "${MM10_FASTA}" "${MM10_CHROMSIZES}" "${L5_PEAKS_BED}" \
...
echo "[$(date)] Stage 3b gao25·l5 complete:"
