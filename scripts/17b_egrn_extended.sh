#!/usr/bin/env bash
# Stage 5 (extended variant) — eGRN using the ORTHOLOGY-EXTENDED cistromes.
# Reuses the Stage 5a/5b adjacencies (tf_to_gene_adj.tsv, region_to_gene_adj.tsv); only the eGRN
# assembly + AUCell differ (--is_extended, cistromes_extended.h5ad). Broader TF coverage (orthology
# motif→TF annotations) at lower confidence than the direct run. Separate outputs (no overwrite).
# Run: bash scripts/17b_egrn_extended.sh
set -euo pipefail
...
echo "[$(date)] extended eGRN complete:"
wc -l "${EREGULON_EXT}"
ls -lh "${AUCELL_EXT}"