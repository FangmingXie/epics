#!/usr/bin/env bash
# Stage 5 extended variant (gao25) — eGRN using the ORTHOLOGY-EXTENDED cistromes (mouse L2/3).
# Reused from scripts/17b.v2.egrn_extended_it.sh; only the work/DB/output paths point at gao25.
...
# Run: bash scripts/17b.gao25.egrn_extended.sh
set -euo pipefail
...
echo "[$(date)] extended eGRN (gao25) complete:"
wc -l "${EREGULON_EXT}"
ls -lh "${AUCELL_EXT}"
