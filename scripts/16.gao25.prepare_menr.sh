#!/usr/bin/env bash
# Stage 4 (gao25) — SCENIC+ prepare_data + motif enrichment → MuData, search space, cistromes.
# Adapts scripts/16.v2.prepare_menr_it.sh. MOUSE edits: --species mus_musculus, the MGI motif table,
# and the mm10 GRCm38 gene annotation. RNA↔ATAC linked by the ___gao25l23 suffix; RNA counts in .X.
...
# Run: bash scripts/16.gao25.prepare_menr.sh   (all steps use the epics env internally)
set -euo pipefail
...
L23_GEX_H5AD=${DATA}/vis_l23_gex.h5ad                     # Stage 1 (raw counts in .X, .raw cleared)
GENE_ANNOT=${DB}/mm10_gene_annotation.tsv                # Stage 2 (GRCm38, UCSC chr names)
MM10_CHROMSIZES_SP=${DB}/mm10_chromsizes_scenicplus.tsv  # Stage 2 (Chromosome/Start/End header)
REGION_SETS=${WORK}/region_sets                          # parent; motif_enrichment iterates its SUBFOLDERS
CTX_RANKINGS=${DB}/gao25_l23.regions_vs_motifs.rankings.feather   # Stage 3b
...
PROJECT_SUFFIX=___gao25l23                                # cisTopic appends this to ATAC barcodes
N_CPU=8
SP="conda run --no-capture-output -n epics scenicplus"

# --- fail-fast ---
for f in "${CISTOPIC_OBJ}" "${L23_GEX_H5AD}" "${GENE_ANNOT}" "${MM10_CHROMSIZES_SP}" "${CTX_RANKINGS}" "${MOTIF_TBL_MGI}"; do
...
# =============================================================================
# 4a — MuData (multiome, per-cell). RNA barcodes get the ___gao25l23 suffix to match ATAC.
...
echo "[$(date)] Stage 4 gao25 complete:"
