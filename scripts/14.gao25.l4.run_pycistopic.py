#!/usr/bin/env python
"""Stage 3c (gao25 · l4) — pycisTopic object FROM the precomputed count matrix + LDA topic modelling.

Fork of scripts/14.gao25.run_pycistopic.py for the L4 layer (plan/06). Method unchanged; only the
npz/gex paths and the PROJECT tag (→ ___gao25l4 barcode suffix) differ. Build the cisTopic object with
...
Run in the epics env:
  conda run --no-capture-output -n epics python -u scripts/14.gao25.l4.run_pycistopic.py
"""
import os
import pickle
import numpy as np
...
DATA = f"{PROJ}/data/gao25/l4"
WORK = f"{DATA}/work"
DB = f"{DATA}/db"

L4_MATRIX_NPZ = f"{WORK}/vis_l4_matrix.npz"                   # Stage 3a: regions×cells CSR + names
L4_GEX_H5AD = f"{DATA}/vis_l4_gex.h5ad"                       # Stage 1: obs carries cell metadata
...
PROJECT = "gao25l4"                                           # → barcode suffix ___gao25l4 (Stage 4/6)
SPLIT_PATTERN = "___"                                         # → tagged cell names 'barcode___gao25l4'
...
for f in (L4_MATRIX_NPZ, L4_GEX_H5AD, MM10_BLACKLIST):
...
# ─────────────────────────────────────────────
npz = np.load(L4_MATRIX_NPZ, allow_pickle=True)
...
print(f"[Stage 3c gao25·l4] matrix regions×cells = {mat.shape[0]:,} x {mat.shape[1]:,}; nnz={mat.nnz:,}")
...
cistopic_obj = create_cistopic_object(
    ...
    project=PROJECT,                     # tag_cells → cell names get '___gao25l4'
...
# ─────────────────────────────────────────────
# 2. Attach cell metadata (indexed by the TAGGED cell names = barcode + '___gao25l4')
# ─────────────────────────────────────────────
import scanpy as sc
obs = sc.read_h5ad(L4_GEX_H5AD, backed="r").obs
...
# ─────────────────────────────────────────────
print(f"[Stage 3c gao25·l4] LDA sweep n_topics={N_TOPICS}, n_iter={N_ITER}")
...
# ─────────────────────────────────────────────
print(f"[Stage 3c gao25·l4] binarizing topics (ntop={NTOP})")
...
print("[Stage 3c gao25·l4] complete.")
