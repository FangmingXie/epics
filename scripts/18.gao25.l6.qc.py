#!/usr/bin/env python
"""Stage 6 (gao25 · l6) — QC / developmental dynamics for the mouse L6 eGRN.

Fork of scripts/18.gao25.qc.py for the L6 layer (plan/06). A SINGLE subclass (L6 IT CTX Glut), so the
...
Outputs:
  ...
  2. Mouse positive-control markers (mixed-case) that surfaced as eRegulons — headline Tbr1 / Foxp2,
     the canonical L6 identity TFs.
...
Run in the epics env:
  conda run --no-capture-output -n epics python -u scripts/18.gao25.l6.qc.py
"""
import os
import numpy as np
import pandas as pd
...
L6_GEX_H5AD = f"{DATA}/vis_l6_gex.h5ad"          # Stage 1 (has roi / age_label / sex / donor)
...
BC_SUFFIX = "___gao25l6"                          # AUCell cell names carry the cisTopic project suffix
...
# mouse L6 positive-control marker TFs (mixed-case); headline Tbr1 / Foxp2 (canonical L6 identity TFs)
KNOWN_TFS = ["Tbr1", "Foxp2", "Tle4", "Sox5", "Foxp1", "Nr4a2", "Bcl11b"]

for f in (EREGULON, AUCELL, L6_GEX_H5AD):
...
ax.set_title("Top 20 eRegulons (red = mouse L6-marker positive control)")
...
print("[QC] Stage 6 gao25·l6 complete.")
