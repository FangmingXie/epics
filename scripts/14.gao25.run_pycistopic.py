#!/usr/bin/env python
"""Stage 3c (gao25) — pycisTopic object FROM the precomputed count matrix + LDA topic modelling.

The core structural change vs wang25: build the cisTopic object with `create_cistopic_object`
(in-memory, from the regions×cells matrix produced in Stage 3a) instead of
`create_cistopic_object_from_fragments`. There are no fragments and no fragment-based QC (TSS/FRiP);
we rely on the Stage-3a peak filter + the h5ad obs QC. Region sets are topics-only (no DAR), per plan.
...
Run in the epics env:
  conda run --no-capture-output -n epics python -u scripts/14.gao25.run_pycistopic.py
"""
import os
import pickle
import numpy as np
...
L23_MATRIX_NPZ = f"{WORK}/vis_l23_matrix.npz"                 # Stage 3a: regions×cells CSR + names
L23_GEX_H5AD = f"{DATA}/vis_l23_gex.h5ad"                     # Stage 1: obs carries cell metadata
...
PROJECT = "gao25l23"                                          # → barcode suffix ___gao25l23 (Stage 4/6)
SPLIT_PATTERN = "___"                                         # → tagged cell names 'barcode___gao25l23'
...
for f in (L23_MATRIX_NPZ, L23_GEX_H5AD, MM10_BLACKLIST):
...
print(f"[Stage 3c gao25] matrix regions×cells = {mat.shape[0]:,} x {mat.shape[1]:,}; nnz={mat.nnz:,}")
...
# ─────────────────────────────────────────────
# 2. Attach cell metadata (indexed by the TAGGED cell names = barcode + '___gao25l23')
# ─────────────────────────────────────────────
import scanpy as sc
...
# ─────────────────────────────────────────────
print(f"[Stage 3c gao25] LDA sweep n_topics={N_TOPICS}, n_iter={N_ITER}")
...
# ─────────────────────────────────────────────
print(f"[Stage 3c gao25] binarizing topics (ntop={NTOP})")
