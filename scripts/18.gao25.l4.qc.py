#!/usr/bin/env python
"""Stage 6 (gao25 · l4) — QC / developmental dynamics for the mouse L4/5 eGRN.

Fork of scripts/18.gao25.qc.py for the L4 layer (plan/06). A SINGLE subclass (L4/5 IT CTX Glut), so the
...
Outputs:
  ...
  2. Mouse positive-control markers (mixed-case) that surfaced as eRegulons — headline Rorb, the
     canonical L4 marker (it formed a cistrome but not an eRegulon in L2/3 — expect it stronger here).
...
Run in the epics env:
  conda run --no-capture-output -n epics python -u scripts/18.gao25.l4.qc.py
"""
import os
import numpy as np
import pandas as pd
...
L4_GEX_H5AD = f"{DATA}/vis_l4_gex.h5ad"          # Stage 1 (has roi / age_label / sex / donor)
...
BC_SUFFIX = "___gao25l4"                          # AUCell cell names carry the cisTopic project suffix
...
# mouse L4/5 positive-control marker TFs (mixed-case); headline Rorb (canonical L4 marker)
KNOWN_TFS = ["Rorb", "Mef2c", "Rfx3", "Satb2", "Cux2"]

for f in (EREGULON, AUCELL, L4_GEX_H5AD):
...
print(f"[QC] HEADLINE — Rorb eRegulon present: {'YES' if 'Rorb' in er_tfs else 'no'} "
      f"(canonical L4 marker; formed a cistrome but not an eRegulon in the L2/3 run)")
...
ax.set_title("Top 20 eRegulons (red = mouse L4/5-marker positive control)")
...
print("[QC] Stage 6 gao25·l4 complete.")
