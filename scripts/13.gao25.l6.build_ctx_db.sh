#!/usr/bin/env bash
# Stage 3b (gao25 · l6) — Build a custom cisTarget motif database on the L6-accessible mouse peaks.
# Fork of scripts/13.gao25.build_ctx_db.sh for the L6 layer (plan/06). The build is species-neutral;
# only the peak BED and the output DB prefix change. Built ON the L6 peak BED (Stage 3a) with a NEW
# prefix gao25_l6 in the per-layer db dir (do NOT share the L2/3 DB — each layer builds on its own peaks).
# MUST run inside the `epics` env (bedtools + cbust + pyarrow):
#   conda run --no-capture-output -n epics bash scripts/13.gao25.l6.build_ctx_db.sh
set -euo pipefail
...
L6_PEAKS_BED=${WORK}/vis_l6_peaks.bed                  # Stage 3a output (chr\tstart\tend)
...
OUT_FASTA=${DB}/l6_regions_1kb_bg.fa                   # padded-background region FASTA
CTX_DB_PREFIX=${DB}/gao25_l6                           # → *.regions_vs_motifs.{rankings,scores}.feather
...
for f in "${MM10_FASTA}" "${MM10_CHROMSIZES}" "${L6_PEAKS_BED}"; do
...
# --- padded-background FASTA for the L6 regions ---
echo "[$(date)] building padded-bg FASTA (${BG_PADDING} bp) for $(wc -l < "${L6_PEAKS_BED}") regions -> ${OUT_FASTA}"
"${DB_BUILDER}/create_fasta_with_padded_bg_from_bed.sh" \
    "${MM10_FASTA}" "${MM10_CHROMSIZES}" "${L6_PEAKS_BED}" \
...
echo "[$(date)] Stage 3b gao25·l6 complete:"
