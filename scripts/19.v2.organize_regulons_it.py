#!/usr/bin/env python
"""Stage 7 (v2) — Organize SCENIC+ eRegulons into a clean regulon -> gene table (all-IT scope).

New copy of 19_organize_regulons.py repointed at the isolated work_it/ dir (plan/04); logic
unchanged. From the Stage 5 direct/extended eRegulon tables, build one long table where each
...
Defaults organize the Stage 5 v2 outputs; --direct/--extended/--out override them.

Run: conda run --no-capture-output -n epics python -u scripts/19.v2.organize_regulons_it.py
"""
import argparse
import os
import sys
...
WORK = f"{PROJ}/data/wang25/work_it"                 # NEW isolated work dir

EREGULON_DIRECT = f"{WORK}/eRegulon.tsv"            # Stage 5c v2 (is_extended=False)
EREGULON_EXTENDED = f"{WORK}/eRegulon_extended.tsv"  # Stage 5c v2 extended (is_extended=True)
...
def main():
    ...
    # 3 — keep only R2G-positive regulons (++ and -+)
    direct = direct[direct["R2G_sign"] == "+"]
    extended = extended[extended["R2G_sign"] == "+"]

    # 4 — dedup by TF: a TF present in direct drops its extended rows entirely
    ...
    # 5 — collapse the multiple region rows per gene into one (regulon, gene) row
    grouped = (
        combined.groupby(
