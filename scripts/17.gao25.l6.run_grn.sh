#!/usr/bin/env bash
# Stage 5 (gao25 · l6) — SCENIC+ GRN inference → eRegulons + per-cell activity (mouse L6).
# Fork of scripts/17.gao25.run_grn.sh for the L6 layer (plan/06). Params are species-agnostic and
# unchanged; only the work/DB paths point at the l6 namespace.
...
# Run: bash scripts/17.gao25.l6.run_grn.sh
set -euo pipefail
...
echo "[$(date)] Stage 5 gao25·l6 complete:"
