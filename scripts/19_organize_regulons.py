#!/usr/bin/env python
"""Stage 7 — Organize SCENIC+ eRegulons into a clean regulon -> gene table.

From the Stage 5 direct/extended eRegulon tables, build one long table where each
...
Defaults organize the Stage 5 outputs; --direct/--extended/--out override them
(e.g. to organize the min_target_genes=5 diagnostic files into a separate table).

Run: conda run --no-capture-output -n epics python -u scripts/19_organize_regulons.py
"""
import argparse
import os
import sys
