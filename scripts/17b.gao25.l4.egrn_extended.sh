#!/usr/bin/env bash
# Stage 5 extended variant (gao25 · l4) — eGRN using the ORTHOLOGY-EXTENDED cistromes (mouse L4/5).
# Fork of scripts/17b.gao25.egrn_extended.sh for the L4 layer (plan/06); only work/DB paths change.
...
# Run: bash scripts/17b.gao25.l4.egrn_extended.sh
set -euo pipefail
...
echo "[$(date)] extended eGRN (gao25·l4) complete:"
wc -l "${EREGULON_EXT}"
ls -lh "${AUCELL_EXT}"
