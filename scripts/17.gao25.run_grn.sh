#!/usr/bin/env bash
# Stage 5 (gao25) — SCENIC+ GRN inference → eRegulons + per-cell activity (mouse L2/3).
# Reused unchanged from scripts/17.v2.run_grn_it.sh (params are species-agnostic); only the
# work/DB/output paths point at gao25.
...
# Run: bash scripts/17.gao25.run_grn.sh
set -euo pipefail
...
echo "[$(date)] Stage 5 gao25 complete:"
